...
public class Dialog_ExtractPowers : Window
{
    ...
    public Dialog_ExtractPowers(List<Tuple<PowerDef, PowerDef>> choices, Pawn pawn,
        Action<Tuple<PowerDef, PowerDef>> onChosen)
    {
    ...
    public override Vector2 InitialSize =>
        new Vector2(LoadedModManager.GetMod<VFEEA_Mod>().GetSettings<VFEEA_ModSettings>().largeWindows, 230f);

    public override void DoWindowContents(Rect inRect)
    {
        inRect = inRect.ContractedBy(15f, 7f);
        Widgets.Label(inRect.TopPartPixels(60f), "VFEEA_ExtractPWR.ExtractChoice".Translate(pawn.NameShortColored));
        inRect.y += 60f;
        foreach (var valueTuple in choices.Zip(Split(inRect, choices.Count, new Vector2(80f, 200f)),
                     (tuple, rect) => new ValueTuple<PowerDef, PowerDef, Rect>(tuple.Item1, tuple.Item2, rect)))
        {
            ...
            onChosen(new Tuple<PowerDef, PowerDef>(item, item2));
            Close();
            break;
        }
    }
